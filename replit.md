...
- "ASM Analysis Current" page (route `/asm-analysis-current`): `src/pages/HpLayoutSql.tsx` — two top-level tabs. "Artifacts" = a table (Health Plan | Submission SQL | File Layout) driven by the `HEALTH_PLANS` array; source files live in `public/files/hp/layout/` (xlsx/docx/csv, download links) and `public/files/hp/sql/` (`.sql`, fetched inline at runtime via `import.meta.env.BASE_URL`). To add more, drop files in those folders and append an entry to `HEALTH_PLANS`; a grain column (Institutional vs Ambulatory) can be added between Health Plan and SQL if needed. "Artifacts Analysis" renders the concept-matrix workbook from `src/data/conceptMatrix.ts` (auto-generated from `Mercy_ASM_Health_Plan_Layout_Concept_Matrix.xlsx`) as summary metrics + nested sub-tabs: Coverage Matrix, Source Field Mapping, Inputs & Notes.
