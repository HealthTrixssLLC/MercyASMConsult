...
- "Findings" page (route `/findings`, `src/pages/Findings.tsx`): consolidated, de-duplicated index of the "Meeting Specific Findings" raised across the Jun 8/10/12 discovery sessions. Data lives in `src/data/findings.ts` as typed `Finding[]` (stable ids `F-01`…, title, detail, tag, `sessions {jun08,jun10,jun12}` booleans, resolution, lane, lanePath). The table shows ID, Finding Detail (title + tag chip + detail), one check/cross presence column per session (`SESSION_COLUMNS`), and a Traceability column (short resolution + wouter `Link` to the Gap & Risk register lane). Dedup is done in the data file: recurring themes merge into one row with multiple session flags (`F-03` = reconciliation framework across all 3 sessions; `F-06` = traceability across Jun 8/10). It is the first item in the "Gap and Risk Register" sidebar group, above the Submission/Reconciliation Lane pages it links to. To add findings, append to `FINDINGS` (re-derive from the `FINDINGS` arrays in `DiscussionJun08/10/12.tsx`).
